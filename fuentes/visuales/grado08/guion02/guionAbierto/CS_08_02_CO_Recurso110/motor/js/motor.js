Motor = new function()
{
    ...
    this.addPagina = function( pregunta, numpagina )
    {
    	...
    	this.audios.push(pagina.audio);
    }
    
    this.creaAudios = function(){
    	this.audios = new Array();	
		for(key in  Motor.datosXML.preguntas)
    	{
    		if(Motor.datosXML.preguntas[key].sonido != ""){
		    	
		        if(Main.navegador == "MSIE 9"){
		        	$('<audio id="audio'+ Motor.datosXML.preguntas[key].id+'" src="'+ pppPreloader.from("data", "data/sonidos/"+Motor.datosXML.preguntas[key].sonido) + '" preload="auto"></audio>')
			        .appendTo($("#mediaHolder"));
				}else{
			    	$('<audio id="audio'+ Motor.datosXML.preguntas[key].id+'" preload="auto"></audio>')
			        .append('<source src="'+ pppPreloader.from("data", "data/sonidos/"+Motor.datosXML.preguntas[key].sonido)+'" type="audio/mpeg" />')
			        .appendTo($("#mediaHolder"));
		        }
	       	}
