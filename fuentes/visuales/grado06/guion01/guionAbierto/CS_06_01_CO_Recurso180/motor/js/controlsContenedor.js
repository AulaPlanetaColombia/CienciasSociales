...
function Boton(_width, _height, src, label, align) {
	...
	if (align == "left")
		...
		this.label.x = (_width - this.label.getMeasuredWidth() ) / 2;
...
// boto de panells inferiors
function BotonDisable(_width, _height, src, label, align) {
...
function moreInfoPanell(texto) {
	...
	btnSrc.src = 'motor/images/tancar.png';
	btnSrc.name = 'tancar';
	...
	this.titol.text = LangRes.lang[LangRes.MASINFORMACION];
	//"Más información";
	this.titol.x = 160;
	this.titol.y = 105;
	...
	/*
	// creem texte de info de more infor
	...
	*/
	...
	createjs.Tween.get(this.contenedor).to({
		alpha : 1
	}, 1250, createjs.Ease.circOut);
}
...
function missatgePanell(texto) {
	...
	this.aceptar = new Boton(144, 28, pppPreloader.from("module", 'motor/images/btnAceptar.png'), LangRes.lang[LangRes.CONTINUAR], "center");
...
// Panell avis en mode exponer
function avisPanell(texto){
	// creem fons del panell
	this.fons = new createjs.Shape();
	this.fons.graphics.beginFill("#f8B334").drawRect(0, 0, 950, 51);
	// creem texte del panell
	...
	this.contenedor.y = 558;
	// creem botons del panell
	this.aceptar = new Boton(144, 28, pppPreloader.from("module", 'motor/images/btnCorregir.png'), LangRes.lang[LangRes.ACEPTAR], "center");
	...
	Main.stage.addChild(this.contenedor);
	this.contenedor.alpha = 0;
	createjs.Tween.get(this.contenedor).wait(500).to({
		alpha : 1
	}, 1250, createjs.Ease.circOut);
}

function avisPanellGuardat(texto){
    // creem fons del panell
    this.fons = new createjs.Shape();
    this.fons.graphics.beginFill("#f8B334").drawRect(0, 0, 950, 51);
    // creem texte del panell
    ...
    var padre = this;
    window.setTimeout(function(){createjs.Tween.get(padre.contenedor).to({
        alpha : 0
    }, 500, createjs.Ease.circOut).call(Contenedor.drawFooter);}, 3500);
    
   /* createjs.Tween.get(this.contenedor).to({
        alpha : 0
    }, 500, createjs.Ease.circOut).call(Contenedor.drawFooter);*/
}

// Panell inferior d ela pantalla de validacio i resultats del motor
function resultatsPanell(numpagines) {
	//creem fons
	this.fons = new createjs.Shape();
	this.fons.graphics.beginFill("#0D3158").drawRect(0, 0, 950, 51);
	//setegem dades paginacio
	this.currentPag = Motor.currentNumPag % 10;
	...
	if (this.numPagines > 1) {// si hi ha més d'una pagina
		//creem poginació
		this.pagines = new Paginador(Contenedor.inici, this.numPagines);
		...
		this.seguent.contenedor.x = 300 + (Math.min(this.numPagines, 10)) * 31 + 40;
		this.seguent.contenedor.y = 12;
		if( this.numPagines<= 10 && this.currentPag == this.numPagines - 1) this.seguent.contenedor.visible = false;
	}
...
resultatsPanell.prototype.selectPag = function(numpag) {
	...
	Contenedor.missatge.seguent.bt.removeAllEventListeners("mousedown");
	// si el numero de paginas es menor que 10
	if (this.numPagines <= 10) {
	...
	// si hay mas de 10 paginas
	if (this.numPagines == Contenedor.inici + this.currentPag + 1) {
		Contenedor.missatge.seguent.label.text = LangRes.lang[LangRes.PRIMERO];
		Contenedor.missatge.seguent.bt.on("mousedown", Contenedor.primeroPaginaHandler);
	} else {
		Contenedor.missatge.seguent.label.text = LangRes.lang[LangRes.SIGUIENTE];
		Contenedor.missatge.seguent.bt.on("mousedown", Contenedor.seguentPaginaHandler);
	}

}
resultatsPanell.prototype.primeroPag = function() {
	this.contenedor.removeChild(this.pagines.contenedor);
...
resultatsPanell.prototype.unselectCurrentPag = function() {
...
resultatsPanell.prototype.checkedCurrentPag = function() {
	this.pagines.pags[this.currentPag].contestada = 1;
	//.bt.gotoAndStop("buit");
}
// per pasar a la pagina seguent
resultatsPanell.prototype.seguentPag = function() {
	var nump = this.numPagines - Contenedor.inici;

	if (this.currentPag < Math.min(nump, 10) - 1)// si no estem a la ultima pagina
	{
		//console.log(nump);
		...
		Contenedor.missatge.seguent.bt
		//Contenedor.missatge.seguent.bt.on("mousedown",Contenedor.seguentPaginaHandler);
...
function solucioPanell(numpagines) {
	...
	if (this.numPagines > 1) {// si tenim més d'una pagina mostrem la paginacio
		//creem paginador
		this.pagines = new Paginador(0, this.numPagines);
...
solucioPanell.prototype.selectPag = function(numpag) {
	...
	if (Motor.pagines[this.currentPag].validacio)
		this.pagines.pags[this.currentPag].numero.color = "#41A62A";
	else
		this.pagines.pags[numpag - 1].numero.color = "#E1001A";
}
...
function PagBase(index, src) {
	...
	this.contenedor.on("mouseover", function(evt) {
		document.body.style.cursor = 'pointer';
	});
	this.contenedor.on("mouseout", function(evt) {
		document.body.style.cursor = 'default';
	});
}

//Bloc de botons de pagines
function Paginador(inici, numpagines) {
	this.contenedor = new createjs.Container();
	this.pags = new Array();
	var counter = 0;
	for ( i = inici; i < numpagines && i < inici + 10; i++)// segons el numero de pagines
	{
		var pag = "";
		//creem tipus de boto de paginas
		if (i == 0)
			pag = new PagBase(i, pppPreloader.from("module", 'motor/images/paginacioIni.png'));
		else if (i == numpagines - 1)
			pag = new PagBase(i, pppPreloader.from("module", 'motor/images/paginacioFi.png'));
		else
			pag = new PagBase(i, pppPreloader.from("module", 'motor/images/paginacio.png'));
		//coloquem pagina
		pag.contenedor.x = counter * 30;
		pag.contenedor.y = 0;
...
function inicialPanellDisable(numpagines){
	//creem fons
	this.fons = new createjs.Shape();
	this.fons.graphics.beginFill("#0D3158").drawRect(0, 0, 950, 51);
	//setegem dades paginacio
	...
	this.contenedor.y = 558;
	//creem botons
	this.validar = new BotonDisable(146, 29, pppPreloader.from("module", 'motor/images/btnGuardar.png'), LangRes.lang[LangRes.GUARDAR], "left");
	this.corregir = new BotonDisable(146, 28, pppPreloader.from("module", 'motor/images/btnValidar.png'), LangRes.lang[LangRes.ENVIAR], "left");
	...
	this.corregir.contenedor.y = 12;
	Contenedor.inici = (Contenedor.inici > 0 ) ? Contenedor.inici : 0;
	this.pagines = "";
	if (this.numPagines > 1) {// si hi ha més d'una pagina
		//creem poginació
		this.pagines = new Paginador(Contenedor.inici, this.numPagines);
...
inicialPanellDisable.prototype = Object.create(inicialPanell.prototype);
...
function inicialPanell(numpagines) {
	...
	this.contenedor.y = 558;
	//creem botons
	this.validar = new Boton(146, 29, pppPreloader.from("module", 'motor/images/btnGuardar.png'), LangRes.lang[LangRes.GUARDAR], "left");
	this.corregir = new Boton(146, 28, pppPreloader.from("module", 'motor/images/btnValidar.png'), LangRes.lang[LangRes.ENVIAR], "left");
...
inicialPanell.prototype.primeroPag = function() {
	this.contenedor.removeChild(this.pagines.contenedor);
