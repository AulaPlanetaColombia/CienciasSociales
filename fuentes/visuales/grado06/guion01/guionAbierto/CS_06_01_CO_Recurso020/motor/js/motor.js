Motor = new function()
{
		...
		if(this.estado != "")
		{
			...
			for(var key in estado)
			{
				for (key2 in Motor.finals) {
					...
					{ 
						console.log(Motor.finals[key2].resposta.idResposta);
    ...
    {   
    	...
    	for(i=0; i < Motor.datosXML.cantidad; i++)
    	{
	        ...
	        pregunta.contenedor.y =  this.INITIAL_Y + this.PREG_Y_PADDING * i +30 + i * ( (this.RESP_HEIGHT  + 20) * ( 8 - Motor.datosXML.cantidad ) ) / (Motor.datosXML.cantidad-1) ;
    ...
    this.drawInicis = function()
    {
    	...
    	for(i=0; i < Motor.preguntes.length; i++)
    	{
	        ...
	        inici.contenedor.y = this.INITIAL_Y + this.RESP_HEIGHT  * i + 20 * i +30+ i * ( (this.RESP_HEIGHT  + 20) * ( 8 - Motor.datosXML.cantidad ) ) / (Motor.datosXML.cantidad-1) ;
    ...
    this.drawFinals = function()
    {
    	...
    	for(i=0; i < Motor.datosXML.cantidad; i++)
    	{
	        ...
	        finali.contenedor.y = this.INITIAL_Y + this.RESP_HEIGHT * i + 20 * i + 30 + i * ( (this.RESP_HEIGHT  + 20) * ( 8 - Motor.datosXML.cantidad ) ) / (Motor.datosXML.cantidad-1)  ;
	        //debugger;
	        // creem caixa resposta
	        var resp = new Resposta( dataResp.text, this.RESP_INNER_WIDTH, this.RESP_INNER_HEIGHT, 5) ;
	        resp.contenedor.x = this.PREG_X_PADDING + this.PREG_FINAL_X_PADDING + 1;
	        resp.contenedor.y = this.INITIAL_Y + this.RESP_HEIGHT * i + 20 * i + 31 + i * ( (this.RESP_HEIGHT  + 20) * ( 8 - Motor.datosXML.cantidad ) ) / (Motor.datosXML.cantidad-1) ;
    ...
    this.preDragAndDrop = function(evt, data)
    {
    	if(Motor.currentElement == null){
    		...
    		console.log(evt.target.parent.x + " - " + evt.target.parent.y);
    		console.log(evt.stageX/Main.scale + " - " + evt.stageY/Main.scale);
    ...
    this.dropResposta = function(evt, data)
    {
    	if(Motor.currentElement == data.element){
	    	...
	    	var trobat = false;
	    	for (key in Motor.inicis) {
				var pt = Motor.inicis[key].contenedor.globalToLocal( ( evt.target.parent.x + 120 ) * Main.scale, ( evt.target.parent.y + 12 ) * Main.scale );
