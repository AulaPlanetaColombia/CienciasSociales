 ...
 * Autor versi�n actual: Javier Garc�s - Sinergia sistemas inform�ticos
 *
 ************************************************************************/

// Inicializaci�n del plugin JQuery
(function($) {
	
	/*
	 * Funci�n a la que debemos llamar desde jquery
	 */
	$.fn.cepdaMediaPlayer = function(options) {
		...
		var defaults = {
			...
			captionsOn: true,							// Mostramos los subt�tulos si los tiene
			autoplayOn: true,							// Reproducimos el v�deo nada m�s crearlo
			bigBtnOn: true,								// Ponemos el bot�n BIG Play
			saveVolumeOn: false,					// No guardamos el nivel de volumen entre sesiones
			nativeSliders: false,					// No usamos sliders nativos
			fullScreenRelocateOn: false,	// Recolocamos en el DOM el elemento para que en fullscreen est� en el body y evite problemas de escalado
		...
		/*
		 * Funci�n para generar un identificador �nico a partir de la fecha y hora
		 * Se usa para generar un Id por cada tag de media (v�deo o audio) que encontremos
		 */
		...
		/*
		* Detecci�n b�sica de algunos aspectos de compatibilidad (pantalla tactil, casos de navegadores determinados, etc)
		*/
		// Detecci�n de Flash (basada en SWFObject 2.2)
      ...
      var SHOCKWAVE_FLASH = "Shockwave Flash",
		...
		// Detecci�n de soporte para LocalStorage (para guardarnos el volumen)
		...
		// No tiene soporte nativo v�deo HTML5
		...
		// No puede usar el modo fullscreen nativo aunque el api lo soporte (por ejemplo, porque quita subt�tulos o controles)
		...
		/*********************************************************************************************************
				MODO VIDEO FLASH (para cuando no hay compatibilidad v�deo nativo HTML5)
		**********************************************************************************************************/

		/* cant_use_html5_video = false; */

		// Miramos si tenemos soporte de v�deo HTML5 nativo
		if (cant_use_html5_video) {
			
			/*
			 * Funci�n principal del plugin cuando ha de usar FLASH (pseudo-constructor)
			 * Se la llamar� para cada elemento video o audio que queramos customizar
			 */
			var cepdaFlashPlayer = function() {
				// Creamos el objeto cepda, que ser� el que usaremos como ra�z para las
				// referencias a todos los elementos a manipular del v�deo/audio
				var cepda = {
					$self: $(this)
				};
				...
				// Contenedor de los media. B�sicamente, lo que se hace es sacar el v�deo/audio del DOM
				// poner este contenedor en su lugar, y dentro de �ste volver a colocar el v�deo/audio
				// junto con los controles, el bot�n BIG Play y otros elementos.
				var $wrapper = $(flashMarkup);
				// Nos aseguramos que sea visible
				//$wrapper.css('visibility','visible');
				
				// Montamos el HTML Markup definitivo
				// A�adimos el contenedor
				...
				*/
				...
				// Lo sacamos del DOM
				$flashObject.detach();
				// Lo volvemos a a�adir tras el tag de v�deo
				cepda.$self.after($flashObject);
				// Nos cargamos el tag de v�deo
				cepda.$self.remove();
			...
			// Hacemos este proceso por todos los elementos que cumplan con el criterio de incializaci�n
			// para que acaben siendo un plugin de Flash y lo dejamos as�.
		...
		**********************************************************************************************************/
		...
		/*
		 * Funci�n principal del plugin (pseudo-constructor)
		 * Se la llamar� para cada elemento video o audio que queramos customizar
		 */
		var cepdaPlayer = function() {
			...
			var wasPlaying; // Indica si se estaba reproduciendo cuando se comenz� el seek
			var fullscreenMode; // Indica si estamos en modo fullscreen
			var hasCaptions;		// Indica que hay subt�tulos
			var captionsActive; // Indica si los subtitulos est�n activos

			// Deshabilitamos el autoplay nativo para usar nuestra opci�n de autoplay
			...
			/*
			 * HTML Markup del bot�n de FullScreen
			 * Si no es un v�deo (es un audio, vaya), queda en blanco
			 */
			...
			// Si en las opciones tenemos una plantilla alternativa (a trav�s de la funci�n onToolbarTemplate),
			...
			// HTML Markup para los subt�tulos
			...
			// Esto se ha de hacer porque iOS no reproduce v�deos que han sido "recolocados".
			// Mirar: http://bugs.jquery.com/ticket/8015
			// - BUG Chrome: Si se clona de v�deo/audio el nodo en vez de simplemente moverlo, �sto provoca que no se carguen
			//   bien los metadatos y el v�deo se quede esperando indefinidamente.
			...
			// A�adimos los controles, el BIG Play (si lo soporta) y la m�scara de cargando
			...
			// El BIG Play, si la opci�n est� activa.
			if (options.bigBtnOn && !cant_show_big_play) {
			...
			// A�adimos finalmente el Markup de subt�tulos
			// que van un poco por libre (subs dentro del contenedors)
			cepda.$controls.after(captionMarkup);

			// Nos quedamos con las referencias a los elementos DOM de los subt�tulos
			 ...
			 * est� siempre sincronizado con el funcionamiento real del v�deo
			 */
				...
				if(!cepda.$self.prop('paused')) {
					...
					if (cepda.$bigBtn) {
						if (cepda.$bigBtn.css('visibility') != 'hidden') {
							...
							}, 700); // Deja un tiempo para que se pueda hacer el efecto fadeout del bot�n
						}
					}
					// Habilitamos el bot�n de fullscreen si es necesario, que por defecto est�
					// inhabilitado hasta que comience la reproducci�n.
			 ...
			 * Aqu� es realmente donde ponemos los estados de los botones de la toolbar, de manera
			 * que siempre est�n sincronizados con la media. As� pues, a�n usando los mandos 
			 * "nativos" del men� contextual (por ejemplo), la toolbar seguir�a sincronizada
			 */
			var startPlayback = function() {
				// Cambiamos el texto y clase del bot�n para que ponga el modo pause
				cepda.$playBtn.text(options.texts.pause).attr('title', options.texts.pauseTitle);
				cepda.$playBtn.addClass('cepda-paused-button');

				// Si todav�a no se ha cargado la metada, mostramos la m�scara de cargando
			...
			var seekUpdate = function() {
				...
				// Si tenemos subt�tulos, los actualizamos
				if(captionsActive) {
					updateCaption();
				}
			};
			 ...
			 * lo que hace que el bot�n activo no pierda el foco.
			 * Esto causa problemas con la selecci�n de subt�tulos.
...
*/
				...
				// forzamos el blur sobre el bot�n de subt�tulos
				blurCaptionBtn();
			};

			/*
			 * Lanzado cuando el usuario acaba de hacer seek manualmente
			 * Si la media estaba reproduci�ndose, la pone en marcha de nuevo
			 */
...
*/
			 ...
			 * Vinculamos eventos, a�adimos el atributo "duration" y generamos el slider jQuery
			 */
			var updateSeek = function() {
				// Obtenemos la duraci�n de la media
				...
				// Quitar el foco del bot�n de subt�tulos manualmente cuando se clique el handle del slider
				$('.ui-slider-handle', cepda.$seek).click(blurCaptionBtn);

				// Vinculamos el avance del v�deo con el elemento DOM de buffer
			...
			/*
			 * Rutina para actualizar el tama�o del buffer en el slider seg�n el progreso de la media
			 */
			...
			/*
			 * Evento de cambio de volumen. Comportamiento del bot�n de volumen y su slider
			 *
			 * Cambiar el volumen a trav�s del slider y usar el localStorage para guardar el volumen entre sesiones
			 */
			 ...
			 * Tambi�n a�adir clases y cambiar el label del volumen en el bot�n
			 */
			...
			var initVolume = function() {
				...
				// quitar el foco manualmente al boton de subt�tulos cuando se pica en el handle del slider de volumen
			 ...
			 *
			 * Cambiar el tama�o del v�deo mientras estamos en modo fullscreen
			 ...
			 * debe normalizar la interficie de usuario para que est� sincronizada.
			 * S�lo nos llega cuando el navegador est� en fullscreen nativo y se sale
			*/
			var fullScreenChange = function(event) {
				// actualizo los elementos visuales a la nueva disposici�n
				$('body').css('overflow', '');
			 ...
			 *
			 * Mira si hay soporte nativo para fullscreen (siempre que mantenga la interfaz y los subt�tulos)
			 * y la aplica, o bien la simula haciendo que el v�deo ocupe toda la ventana
			...
			*/
			var goFullscreen = function() {
				...
				// Miramos qu� versiones de navegador soporta fullscreen
				
				// Si estamos en modo fullscreen, salimos de �ste
				if(fullscreenMode) {
				...
				} else { // Si no est�bamos en modo fullscreen, entramos en �ste
				  ...
				  if(cepda.$self[0].mozRequestFullScreen && supportsFullscreen) {
					...
					// entramos fullscreen: actualizmo los elementos visuales a la nueva posici�n
			...
			/*
			 * Comportamiento de los subt�tulos
			 *
			 * Desactivando los subt�tulos cuando falle la carga...
			 */
			var captions;
			...
			/*
			 * Actualizar los subt�tulos en base a "currentTime"
			 * Obtenido y adaptado del art�culo "Accessible HTML5 Video with JavaScripted captions" de Bruce Lawson�
			...
			var updateCaption = function() {
				...
				// Buscamos el subt�tulo que nos toca mostrar
				...
				// lo ponemos (si no encontramos ninguno, entonces "text" quedar� vac�o)
			 ...
			 * Recibe la url de los subt�tulos como par�metro de entrada
			 */
			var loadCaption = function(url) {
				// A�adimos la clase de "cargando" para que aparezca el indicador
				...
				var jqXHR = $.ajax({
					...
					success: function(data) {
						...
						// Ahora indicamos que hay que recalcular la medida de la toolbar, al a�adir el bot�n
						// de subt�tulos
						if (options.onToolbarChanged)
							options.onToolbarChanged(cepda, options);
							
						// Mostramos los subt�tulos
						cepda.$caption.show();
						captionsActive = true;

						// En caso de que la media est� en pausa, actualizamos los subt�tulos para que se vean los actuales
						if(cepda.$self.prop('paused')) {
							updateCaption();
						}
					...
					error: function() {
						// Si hay alg�n error, desactivo el bot�n de subt�tulos
						captionOff();
						// Aqu� podr�amos mostrar un mensaje al usuario, si acaso
					}
				});
			};

			/*
			 * Inicializaci�n y carga de subt�tulos
			 */
			var initCaption = function() {
				...
				// Si hay al menos un elemento DOM <track>, mostramos el bot�n
				if(cepda.$track.length) {
					cepda.$captionBtn.show();
					// Avisamos para que pongan la toolbar a la medida que toque
					// al a�adir el nuevo bot�n
					if (options.onToolbarChanged) {
						options.onToolbarChanged(cepda, options);
					}
				} else {
				...
				// Miramos si hay un track de subt�tulos (el primero)
				if(cepda.$track.length) {
					
					hasCaptions = true;
					
					// Si s�lo hay un elemento <track>
					// cargamos directamente los subt�tulos al picar sobre el bot�n de subt�tulos
					...
					// Cargamos los subt�tulos por defecto si tenemos la opci�n activa
			...
			/*
			 * Funci�n de inicializaci�n general (la llama el constructor)
			...
			var init = function() {
				...
				// Eventos del propio v�deo
				cepda.$self.bind('play', startPlayback);
				cepda.$self.bind('pause', stopPlayback);
				cepda.$self.bind('ended', stopPlayback);

				// Evento de actualizaci�n de tiempo para el slider (que se vaya actualizando)
				cepda.$self.bind('timeupdate', seekUpdate);

				// Eventos del modo fullscreen (por defecto el bot�n est� desactivado, hasta que se empiece a reproducir el v�deo)
				cepda.$fullscreenBtn.prop('disabled',true);
				cepda.$fullscreenBtn.click(goFullscreen);

				// Inicializaci�n del volumen (bot�n y slider)
				initVolume();
				...
				// Inicializaci�n del slider de seek
				if (!options.nativeSliders)
				initSeek();
				...
				cepda.$self.bind('loadedmetadata', function() {
					 ...
					 * efectivamente la informaci�n de la duraci�n est� disponible.
					 */
				...
				// Quitamos los controles nativos del v�deo/audio
				cepda.$self.prop('controls', false);

				// Si es audio, a�adimos la clase "audio-player"
				if(cepda.$self.is('audio')) {
					/*
					 * Si la media es de tipo <audio>, a�adimos la clase "audio-player".
				...
				// Forzamos la actualizaci�n del slider de seek la primera vez (para iOS)
				if (is_ios_browser) {
					updateSeek();
				}
				if (options.onToolbarChanged)
				...
				// Si est� en modo autoplay, ponemos el v�deo en marcha
				// (excepto para iOS, que no deja)
				if (options.autoplayOn && !is_ios_browser) {
					playMedia();
				}

				// A�adio API PUBLIC al objeto CEPDA para cambiar el v�deo
				...
				cepda.loadVideo = function(videoUrl, srtUrl, srtLang, srtLabel) {
					// Paramos el v�deo si est� reproduci�ndose
					if (this.isPlaying())
						this.playPause();
					// Ahora cargamos el nuevo v�deo
					...
					// Lanzamos la carga del v�deo (en 100 ms).
					this.$self[0].load();
