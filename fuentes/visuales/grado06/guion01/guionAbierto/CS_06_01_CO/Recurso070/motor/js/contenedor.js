...
Contenedor = new function()
{
	...
	this.cargarDatos = function()
	{
		$.get(pppPreloader.from("data", "data/datos.xml" + "?time=" + new Date().getTime()), function (xml) {
	        ...
	        Contenedor.datosXML.enunciado = enunciadoParseado;
	       	...
	       	var enunciado = motor.children('enunciado');
	        Contenedor.datosXML.enunciado = enunciado.text();
	 ...
	 this.settingsCalc = function(){
	 	...
	 	if(Contenedor.datosXML.plataforma.grado == 1)
		{
			...
			//$('.calculator-close').on("click", function(){ Contenedor.closeHandler();});
		}
		else{
	...
	{
	   	if( evt.primary ){
	      	if(Contenedor.missatge.seguentPag())
	      		Motor.ponerPagina( Motor.currentNumPag + 2 );
	     }
	}
	//Event del boto de seguent pagina
	this.primeroPaginaHandler = function(evt)
	{
	   	if( evt.primary ){
	      	Contenedor.missatge.primeroPag();
	      	Motor.ponerPagina( 1 );
	     }
	}
	...
	this.selectPagHandler = function(evt)
	{
		if( evt.primary ){
			...
			Contenedor.missatge.selectPag(this.index);
	...
	this.validaHandler = function(evt){
		if( evt.primary ){
			// enviem el resultat per scorm
			if( Scorm.modo == Scorm.MODO_EXAMEN ){
				//s'envien les dades a scorm si esta en mode EXAMEN
				var temps = Contenedor.getTemps(); 
				var estado = Motor.getEstado();
				aulaPlaneta.SCORM.ejercicio_guardar( estado );
				Contenedor.missatge = new avisPanellGuardat(LangRes.lang[ LangRes.MSG_GUARDADOCORRECTO ]);
	...
	this.drawWarning = function(){
		Contenedor.missatge = new avisPanell( LangRes.lang[ LangRes.MSG_BROWSEMODE ] /*"Has agotado el tiempo. Si continúas, se almazenará la nota del ejercicio. Si reinicias, empezarás el ejercicio de nuevo"*/ );
		Contenedor.missatge.aceptar.bt.on("mousedown", Contenedor.exponerHandler );
	}

	this.exponerHandler = function(evt){
		if( evt.primary ){
			//borramos el mensaje
			Main.stage.removeChild( Contenedor.missatge.contenedor );
			
			//barra controls inicial
			Contenedor.drawFooterExponer();
		}
	}
